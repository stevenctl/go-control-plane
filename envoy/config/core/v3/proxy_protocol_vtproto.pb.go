...
func (m *ProxyProtocolConfig) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	...
	if len(m.CustomTlvs) > 0 {
		for iNdEx := len(m.CustomTlvs) - 1; iNdEx >= 0; iNdEx-- {
			size, err := m.CustomTlvs[iNdEx].MarshalToSizedBufferVTStrict(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
